...
- `JWT_SECRET` (или `JWT_SHARED_SECRET`), `JWT_ISSUER`, `JWT_AUDIENCE`
- `KEYBUNDLE_REQUIRE_ALLOWLIST` (по умолчанию `false`; включайте `true`, чтобы разрешать выдачу ключей только по allowlist/членству в чате), `REPLAY_FALLBACK_CAP`
