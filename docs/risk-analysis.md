...
3. **Доступ к публичным ключам по userId.**
   - **Статус:** ограничено. Добавлены поля `allowAnyRequester` и `allowedRequesters`, а также проверка членства
     в указанном чате (при включённом режиме allowlist). По умолчанию allowlist отключён для упрощения первичной инициации,
     но его можно включить через `KEYBUNDLE_REQUIRE_ALLOWLIST=true` или `config/keybundle.requireAllowlist`.
   - **Рекомендации:** расширить клиентскую часть для управления allowlist, логировать неудачные попытки доступа и документировать
     требования включения режима на продакшене.
