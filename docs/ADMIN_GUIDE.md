...
- Если `KEYBUNDLE_REQUIRE_ALLOWLIST` включён (`false` по умолчанию для упрощения первых контактов), поддерживайте allowlist для выдачи ключей через обновление KeyBundle (добавляйте
