...
- Регулярно меняйте значение `JWT_SECRET`/`JWT_SHARED_SECRET` и перезапускайте сервер, чтобы аннулировать старые токены.
- Используйте поле `tokenVersion` (через административный скрипт или при смене пароля), чтобы отозвать активные JWT: при инкременте
  значения все ранее выданные токены станут недействительными.
...
- Если `KEYBUNDLE_REQUIRE_ALLOWLIST` включён, поддерживайте allowlist для выдачи ключей через обновление KeyBundle (добавляйте
  `allowedRequesters` или включайте `allowAnyRequester` только для доверенных сценариев).
