...
test('HS256 tokens are accepted when shared secret configured', () => {
  withSharedSecret('unit-test-secret', () => {
    const token = jwt.sign({ sub: 'hs-user', userId: 'hs-user', tokenVersion: 0 }, 'unit-test-secret', {
      algorithm: 'HS256',
      expiresIn: '5m',
      audience: 'test-aud',
      issuer: 'test-issuer',
...
test('HS256 tokens use JWT_SECRET env for backwards compatibility', () => {
  withLegacySecret('legacy-secret', () => {
    const token = jwt.sign({ sub: 'legacy-user', userId: 'legacy-user', tokenVersion: 0 }, 'legacy-secret', {
      algorithm: 'HS256',
      expiresIn: '5m',
      audience: 'legacy-aud',
...
test('tokens signed with unsupported algorithm are rejected', () => {
  withSharedSecret('unit-test-secret', () => {
    const token = jwt.sign({ sub: 'bad-alg', tokenVersion: 0 }, 'unit-test-secret', {
      algorithm: 'HS384',
      expiresIn: '5m',
      audience: 'test-aud',
      issuer: 'test-issuer',
